...
function EditProfileModal({ isOpen, onClose, profileData, onUpdate }) {
  const [formData, setFormData] = useState({
    ...
    license_no: profileData?.guide?.license_no || "",
  });
  const [image, setImage] = useState(null);
  const [preview, setPreview] = useState(profileData?.user?.profile_image || "");
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[#1A2235] border border-gray-800 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden shadow-black">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="flex flex-col items-center gap-4">
             <div className="w-24 h-24 rounded-full border border-gray-700 overflow-hidden bg-black flex items-center justify-center relative group">
                {preview ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-[0.65rem] font-bold text-gray-500 uppercase">Phone</label>
          ...
          <div className="flex gap-4 pt-4">
             <button type="submit" disabled={loading} className="flex-1 py-4 bg-[#AAFF00] text-[#0A0F1C] font-bold uppercase text-xs tracking-widest rounded-lg hover:bg-white transition-all disabled:opacity-50 shadow-lg shadow-[#AAFF00]/10">
